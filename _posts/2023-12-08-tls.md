...
So basically, keystore and trustore are archives of CA, certificates, or
private keys. Depending on the content of this archive, a keystore is classfied
into one of three different types: `KeyStore.PrivateKeyEntry`,
`KeyStore.SecretKeyEntry` and `KeyStore.TrustedCertificateEntry`. See
...
needs to establish a TLS connection with a client, it displays this certficate
